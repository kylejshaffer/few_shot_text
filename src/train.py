...
class Trainer:
    def __init__(self, args, train_data, test_data, vocab_size):
        ...
        self._setup_data()
        self.model = SiameseNet(args, vocab_size=self.vocab_size).to(device)
    ...
    def evaluate(self, data):
        ...
        for ix, (x_left, x_right, y) in enumerate(data):
            ...
            with torch.no_grad():
                ...
                probs = F.sigmoid(output).data.cpu().numpy().tolist()
                # print(type(probs))
    ...
    def train(self):
        ...
        for epoch in range(args.n_epochs):
            ...
            for x_left, x_right, y_batch in self.train_loader:
                print('=', flush=True, end='')
                x_left, x_right, y_batch = x_left.to(device), x_right.to(device), y_batch.to(device)
            ...
            if test_acc > best_acc:
                print('Saving model...')
                best_acc = test_acc
                torch.save(self.model.state_dict(), os.path.join(self.model_path, 'stacked_one_shot_model.pt'))
